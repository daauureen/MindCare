...
import { fmt, hoursLeft } from '../lib/utils.js';
import { Area, Top, Empty, Profile } from '../components/common.jsx';
import { DocItem } from '../components/documents.jsx';
import { adminDecide, hideTest, blockUser } from '../lib/db.js';

// Панель администратора: заявки, пользователи, модерация тестов, статистика.
// Все мутации идут в Supabase, после — reload() снимка.

export function AdminApp({ me, db, reload, route, go, notify, logout }) {
  const pending = db.users.filter((u) => u.role === "PSYCHOLOGIST" && ["PENDING", "NEEDS_MORE_DOCS"].includes(u.profile?.verification_status));
  ...
  const decide = async (userId, status, reason) => {
    ...
    await adminDecide(userId, status, reason, me.id);
    await reload();
    notify(titles[status]);
    go("queue");
  };

  const onHideTest = async (testId) => {
    const reason = window.prompt("Причина скрытия теста");
    if (!reason) return;
    await hideTest(testId, reason, me.id);
    await reload();
    notify("Тест скрыт");
  };
  ...
  const onBlockUser = async (userId) => {
    ...
    await blockUser(userId, me.id, reason);
    await reload();
  };
  ...
  if (route.n === "request") {
        ...
        <div className="body stack">
          {db.users.filter((u) => u.role !== "ADMIN").map((u) => (
            <div key={u.id} className="card between">
              <div><h3>{u.full_name}</h3><p className="tiny" style={{ marginTop: 4 }}>{u.email} · {u.role === "STUDENT" ? "студент" : "психолог"}{u.status === "BLOCKED" ? " · заблокирован" : ""}</p></div>
              <button className="btn quiet sm" onClick={() => onBlockUser(u.id)}>{u.status === "ACTIVE" ? "Заблокировать" : "Снять блок"}</button>
            </div>
          ))}
        </div>
      </>
        ...
        <div className="body stack">
          {db.tests.map((t) => (
            <div key={t.id} className="card">
              ...
              <p className="tiny" style={{ marginTop: 6 }}>{db.users.find((u) => u.id === t.author_id)?.full_name} · {t.questions.length} вопросов</p>
              {t.status === "PUBLISHED" && <button className="btn quiet sm" style={{ marginTop: 10 }} onClick={() => onHideTest(t.id)}>Скрыть</button>}
            </div>
          ))}
        </div>
      </>
    ...
    const st = {
      ...
      "Подтверждено психологов": db.users.filter((u) => u.profile?.verification_status === "APPROVED").length,
      ...
      "Просмотрено психологами": db.attempts.filter((a) => a.review_status && a.review_status !== "NEW").length,
    };
      ...
      <>
        ...
        <div className="body stack">
          ...
          {db.audit.slice(0, 12).map((l) => (
            <p key={l.id} className="tiny">{fmt(l.created_at)} · {l.action}{l.reason ? ` · ${l.reason}` : ""}</p>
          ))}
        </div>
      </>
    );
    ...
    content = (
      <>
        ...
        <div className="body stack">
          ...
          {pending.map((u) => {
            const left = hoursLeft(u.profile.submitted_at || new Date().toISOString());
            return (
              <button key={u.id} className="card" style={{ textAlign: "left", width: "100%", cursor: "pointer" }} onClick={() => go("request", { id: u.id })}>
                <div className="between"><h3>{u.full_name}</h3>
                  <span className={"tag " + (left <= 4 ? "warn" : "")}>{u.profile.verification_status === "NEEDS_MORE_DOCS" ? "ждём документы" : left > 0 ? `${left} ч на решение` : "срок вышел"}</span></div>
                <p className="tiny" style={{ marginTop: 8 }}>
                  {u.profile.specializations.join(" · ")} · {u.profile.documents.length} документ(ов)
                  {!u.email_verified_at && " · email не подтверждён"}
                </p>
              </button>
            );
          })}
    ...
    <>
      ...
      <div className="body stack">
        ...
        <div className="row" style={{ gap: 6, flexWrap: "wrap" }}>
          ...
          <span className={"tag " + (u.email_verified_at ? "" : "warn")}>{u.email_verified_at ? "email подтверждён" : "email не подтверждён"}</span>
        </div>
        <div className="card"><div className="eyebrow">Образование</div><p style={{ marginTop: 8, fontSize: 14 }}>{p.education}</p></div>
        <div className="card"><div className="eyebrow">Специализация и опыт</div><p style={{ marginTop: 8, fontSize: 14 }}>{p.specializations.join(" · ")} · {p.experience_years} лет</p></div>
